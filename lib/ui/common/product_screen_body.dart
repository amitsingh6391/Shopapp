...
class _ProductScreenBodyState extends State<ProductScreenBody> {
  ...
  void _scrollListener() {
    print("this is scrolllistner****************************");
    if (scrollController.offset >= scrollController.position.maxScrollExtent &&
        !scrollController.position.outOfRange) {
      print("at the end of list");

      // allProductsBloc.productRepository.getAllProducts(2);

      context.read<AllProductsBloc>().add(
            const AllProductsEvent.getNextProductsData(2),
          );

      // (context) => getIt<AllProductsBloc>()
      //   ..add(
      //     const AllProductsEvent.getNextProductsData(2),
      //   );
    ...
    return Center(
      child: BlocConsumer<AllProductsBloc, AllProductsState>(
        listener: (context, productState) {
          // if (productState is AllProductsState.loadInProgress()) {
          //   ScaffoldMessenger.of(context)
          //       .showSnackBar(SnackBar(content: Text(productState.message)));
          // } else if (productState is BeerSuccessState &&
          //     productState.beers.isEmpty) {
          //   ScaffoldMessenger.of(context)
          //       .showSnackBar(SnackBar(content: Text('No more beers')));
          // } else if (productState is BeerErrorState) {
          //   ScaffoldMessenger.of(context)
